...
+ (NSString *)encodedData:(NSString *)srcData
{
    ...
    int yu = cstrOriBodyLen%8;
    if (yu != 0) {
        cstrOriBodyLen += (8-yu);
    }
    
    char *chdddBody = (char*)malloc(cstrOriBodyLen);
    memset(chdddBody, 0, cstrOriBodyLen);
    strlcpy(chdddBody, cstrOriBody, cstrOriBodyLen);
    
    char *chEncodeBody = (char*)malloc(cstrOriBodyLen);
    memset(chEncodeBody, 0, cstrOriBodyLen);
    
    int encodelen = 0;
    encodelen = AES_Encrypt_1((const unsigned char*)chdddBody,
                              cstrOriBodyLen,
...
+ (NSString *)decodeData:(NSString *)srcData
{
    ...
    NSData *encodedData = [[NSData alloc] initWithData:[srcData dataUsingEncoding:NSUTF8StringEncoding]] ;
    ...
    NSString *bodyMsg = [[NSString alloc] initWithData:(__bridge NSData *)bodyData encoding:NSUTF8StringEncoding];
